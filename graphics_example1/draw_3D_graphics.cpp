...
// library *.lib file (ie Vertices.lib).
{
	...
	static mesh m1("Arm.x"); // the mesh here gets constructed only once since  its a static local variable, so no need to put in the initialization section 
	static mesh m2("plane.x");//You can keep adding max of 8 mesh files.
	static mesh m3("Arm.x");
	...
	if( KEY(VK_UP) ) z += dz;//increment position of z. Make sure there is Pz = ...+z

	if( KEY(VK_DOWN) ) z -= dz; //Decrement the position of z

	if (KEY(VK_RIGHT)) x += dx;//increment position of X

	if (KEY(VK_LEFT)) x -= dx; //Decrement the position of x.  Make sure Px= ...x
